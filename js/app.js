...
function hitungMundur() {
    const targetWaktu = new Date(document.getElementById('tampilan-waktu').getAttribute('data-waktu')).getTime();
    const now = new Date().getTime();
    const selisih = targetWaktu - now;
    ...
    if (selisih < 0) {
        ...
        document.getElementById('tampilan-waktu').innerHTML = '<h2>Waktu Telah Berlalu</h2>';
    }
}

hitungMundur(); // Panggil fungsi untuk pertama kali
const interval = setInterval(hitungMundur, 1000); // Setiap 1 detik akan dipanggil kembali fungsi hitungMundur
const buka = async () => {
